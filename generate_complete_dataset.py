...
"""
    ...
    async def _generate_real_mcp_messages(self, user_question: str) -> List[Dict]:
        """生成真实的MCP调用消息"""
        try:
            # 智能选择工具
            tool_name, parameters = self._infer_tool_from_question(user_question)
            
            if not tool_name:
                tool_name = "search_chain_data"
                parameters = {"query": "查询"}
            
            logger.info(f"选择工具: {tool_name}, 参数: {parameters}")
            
            # 真实调用MCP工具
            mcp_result = await self.mcp_client.call_tool(tool_name, parameters)
            logger.info("=== MCP调用结果 ===\n" + json.dumps(mcp_result, ensure_ascii=False, indent=2) + "\n=== 结束 ===")
            
            # 提取真实的结果数据
            observation_value = json.dumps(mcp_result.get("result", {}), ensure_ascii=False)
            
            messages = [
                {
                    "from": "function_call",
                    "value": json.dumps({"name": tool_name, "arguments": parameters}, ensure_ascii=False)
                },
                {
                    "from": "observation",
                    "value": json.dumps({"content": [{"type": "text", "text": observation_value}]}, ensure_ascii=False)
                }
            ]
            ...
            logger.error(f"生成MCP消息失败: {e}")
            return []
    
    def _infer_tool_from_question(self, question: str) -> tuple:
        """从问题推断合适的工具和参数"""
        ...
        # 提取交易哈希
        tx_hash_match = re.search(r'0x[a-fA-F0-9]{64}', question)
        tx_hash = tx_hash_match.group(0) if tx_hash_match else None
        
        # 工具推断规则
        if address:
            if any(word in question_lower for word in ["转账", "转移", "transfer", "持仓变化"]):
                return "list_address_latest_token_transfers", {"address": address}
            elif any(word in question_lower for word in ["交易记录", "最近交易"]):
                return "list_address_latest_txs", {"address": address}
            elif any(word in question_lower for word in ["资产", "价值", "总额"]):
                return "query_asset_value_by_address", {"address": address}
            elif any(word in question_lower for word in ["持仓", "余额", "持有"]):
                return "query_token_holding_by_address", {"address": address}
            else:
                return "get_address_details_by_address", {"address": address}
        
        elif tx_hash:
            return "get_tx_by_hash", {"hash": tx_hash}
        
        elif any(word in question_lower for word in ["搜索", "查找"]):
            # 提取搜索关键词
            query = question
            return "search_chain_data", {"query": query}
        
        elif any(word in question_lower for word in ["区块"]):
            # 提取区块号
            block_match = re.search(r'\d+', question)
            if block_match:
                return "get_block_by_number", {"number": block_match.group(0)}
            else:
                return "list_latest_blocks", {"timestamp": str(int(time.time()))}
        
        # 默认使用搜索
        return "search_chain_data", {"query": question}
